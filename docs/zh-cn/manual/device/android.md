...
::: danger 注意
已知在部分实体设备上 MAA 的多账号（账号切换）功能可能出现异常，无法工作。
由于实体设备型号极多，情况复杂，且 MAA 主要在电脑端模拟器环境中工作，所以该问题并无计划修复。
如果你遇到了该问题，可以尝试根据设备情况调大/调小 `手机设置→开发者选项→最小宽度`，或使用命令 `adb shell wm density <值>`。
注意调整该值时，**切勿调的过大或过小，强烈建议在** `[100-800]` **区间内，以防设备出现黑屏/卡开机等故障。**
**该操作有风险，请确保你知道自己在做什么，如调整后设备无法正常使用后果自负。**
强烈建议在**下次重启设备前**还原该设置，如忘记具体值可使用命令 `adb shell wm density reset`。
:::
